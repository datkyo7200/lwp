...
class MPSLOptionsFactory {
    ...
    static function createControl(&$option, $parent = '', $type = 'default') {
        ...
        switch($option['type']) {
            ...
            case 'tiny_mce' : self::addLabel2($option); self::addEditorTextarea($option); break;
     ...
     * @since 2.2.0
     */
    static function addEditorTextarea(&$option) {
        ?>
        <textarea id="<?php echo self::$altPrefix . $option['name']; ?>"></textarea>
        <?php
    }
